...
private:
	...
	void SiftDown(int pos)
	{
		...
		int largest = pos;
		if(l < size)			//Find the largest in pos and its child nodes
			if(heapArray[l] > heapArray[largest]) largest = l;
		if(r < size)
			if(heapArray[r] > heapArray[largest]) largest = r;
		if (largest != pos)		//Once there is nodes swapping, the property of
		{						//the heap might also change
			swap(heapArray, pos, largest);		//So we have to continue to SiftDown()
			if (!IsLeaf(largest))
				SiftDown(largest);
		}
	}
public:
	MaxHeap(T* arr, int n, int max)
	...
	void insert(const T& it)
	{
		...
		size++;
		heapArray[curr] = it;
		//sift up until curr's parent is larger than curr
...
template<typename T>
inline void MaxHeap<T>::swap(T* arr, int pos1, int pos2)
{
	T temp = arr[pos1];
	arr[pos1] = arr[pos2];
	arr[pos2] = temp;
